...
pub fn remove_subtrees(tree: &mut Tree, roots: &[u32]) -> RemovedSummary {
    ...
    for &r in roots {
        if tree.node(r).is_none() {
            continue; // Defensive: caller validates; ignore unknown ids.
        }
...
fn compact_order(tree: &mut Tree) {
    ...
    for (dir_id, extra) in extras.iter_mut().enumerate() {
        let n = &tree.arena[dir_id];
        if !n.is_dir() || n.dir_index == u32::MAX || n.is_removed() {
        ...
        let kept: Vec<u32> = old
            ...
            .filter(|&id| {
                tree.arena
                    .get(id as usize)
                    .map_or(true, |n| !n.is_removed() && n.on_disk > 0)
            })
            .collect();
